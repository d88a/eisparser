"""Background worker for automatic Stage 1-4 execution."""
...
class WorkerService:
    """Runs Stage 1-4 in a periodic loop."""

    DEFAULT_LOCK_KEY = 704127301905221
    ...
    def _reg_numbers_by_status(self, status: str, limit: int) -> list[str]:
        items = self.pipeline.db.zakupki.get_by_status(status)
        items = sorted(
            items,
            key=lambda z: (
    ...
    def run_cycle(self, cycle_number: int):
        ...
        if not self.enable_stage4:
            ...
            stage4_reg_numbers = self._reg_numbers_by_status("url_ready", self.limit)
            if stage4_reg_numbers:
                self._run_stage4_batch(stage4_reg_numbers)
            else:
                self.logger.info("Stage 4 skipped: no purchases with status url_ready")
    ...
    def run_forever(self, max_cycles: Optional[int] = None) -> int:
        ...
        self.logger.info(
            "Worker started: interval=%ss limit=%s top_n=%s details=%s stage4_enabled=%s",
            self.interval,
            self.limit,
            self.top_n,
            self.get_details,
